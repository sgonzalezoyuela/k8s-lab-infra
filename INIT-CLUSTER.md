...
For a lab, the simplest path is a token bound to `root@pam` with **Privilege Separation = false**.
For a more constrained setup, the token's role needs at least:
...
# 3. Run the full pipeline (idempotent)
just cluster-up
...
| `NETWORK_CIDR` | Subnet mask length used in Talos addresses (e.g. `24`). **Must put gateway in the same subnet** unless you add a custom on-link route. |
| `NETWORK_GATEWAY` | Default gateway IP — must be reachable from the node IPs given the CIDR above |
...
| Default route fails after install | Gateway not in same subnet as node IP at the configured `NETWORK_CIDR` | Either widen `NETWORK_CIDR` so gateway is on-link, or change the node IPs to share a subnet with the gateway |
