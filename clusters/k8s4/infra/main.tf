# k8s4 cluster wrapper for the shared OpenTofu module under tools/infra.
#
# This file declares the Proxmox provider (which the shared module no longer
# does) and forwards every input variable to the module. State for this cluster
# lives in this directory's terraform.tfstate; resource addresses are namespaced
# under module.cluster.* via `tofu state mv` during the migration.
