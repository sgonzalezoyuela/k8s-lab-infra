...
# Inputs from the shared tools library:
PATCHES_DIR="$TOOLS_DIR/talos/patches"
# Outputs go to $PWD/_out (cluster directory).
...
envsubst "$allowlist" < "$PATCHES_DIR/cp.yaml.tpl"  > _out/patches/cp.yaml
envsubst "$allowlist" < "$PATCHES_DIR/wk0.yaml.tpl" > _out/patches/wk0.yaml
